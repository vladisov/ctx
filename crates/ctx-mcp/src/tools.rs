...
pub async fn call_tool(
    ...
    match tool_name {
        ...
        "ctx_packs_preview" => {
            ...
            let mut result = server.renderer.render_request(RenderRequest {
                pack_ids,
            }).await?;
        ...
        "ctx_packs_snapshot" => {
             ...
             let result = server.renderer.render_request(RenderRequest {
                 pack_ids,
             }).await?;
             ...
             let snapshot = ctx_core::Snapshot::new(
                 ...
                 blake3::hash(result.payload.clone().unwrap_or_default().as_bytes()).to_hex().to_string(),
                 label
             );
...
pub fn list_tools(read_only: bool) -> serde_json::Value {
    let tools = vec![
        ...
        tool_schema("ctx_packs_get", "Get pack details", json!({
            "type": "object",
            "properties": {
        ...
        tool_schema("ctx_packs_preview", "Preview pack rendering", json!({
            "type": "object",
            "properties": {
                "packs": {
                    "type": "array",
        ...
        tool_schema("ctx_packs_snapshot", "Create snapshot of rendered packs", json!({
            "type": "object",
            "properties": {
